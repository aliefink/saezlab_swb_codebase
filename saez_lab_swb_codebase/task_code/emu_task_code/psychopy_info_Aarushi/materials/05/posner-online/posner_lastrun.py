...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='G:\\Shared drives\\Science\\books\\BuildingExperiments_Ed2\\materials\\05\\posner-online\\posner_lastrun.py',
...
instructText = visual.TextStim(win=win, name='instructText',
    ...
    languageStyle='LTR',
    depth=0.0);
instructEnd = keyboard.Keyboard()

# Initialize components for Routine "ITI"
ITIClock = core.Clock()
fixationITI = visual.ShapeStim(
    win=win, name='fixationITI', vertices='cross',units='height', 
    size=(0.05, 0.05),
    ori=0, pos=(0, 0),
    lineWidth=2,     colorSpace='rgb',  lineColor='white', fillColor='white',
...
target = visual.Rect(
    ...
    ori=0, pos=[0,0],
    lineWidth=1,     colorSpace='rgb',  lineColor='SeaGreen', fillColor='MediumSeaGreen',
...
instructComponents = [instructText, instructEnd]
...
while continueRoutine:
    ...
    # *instructEnd* updates
    waitOnFlip = False
    if instructEnd.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        instructEnd.frameNStart = frameN  # exact frame index
        instructEnd.tStart = t  # local t and not account for scr refresh
        instructEnd.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(instructEnd, 'tStartRefresh')  # time at next scr refresh
        instructEnd.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(instructEnd.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(instructEnd.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if instructEnd.status == STARTED and not waitOnFlip:
        theseKeys = instructEnd.getKeys(keyList=None, waitRelease=False)
        _instructEnd_allKeys.extend(theseKeys)
        if len(_instructEnd_allKeys):
            instructEnd.keys = _instructEnd_allKeys[-1].name  # just the last key pressed
            instructEnd.rt = _instructEnd_allKeys[-1].rt
...
for thisTrial in trials:
    ...
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    trialComponents = [cue, target, key_resp_2]
    ...
    while continueRoutine:
        ...
        # *key_resp_2* updates
        waitOnFlip = False
        if key_resp_2.status == NOT_STARTED and frameN >= 12:
            # keep track of start time/frame for later
