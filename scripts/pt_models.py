...
def negll_base_pt_pyEM(params, subj_df, prior=None, output='npl'):
    ...
    if output == 'npl':
        ...
        subj_dict = {'subj_id'     : subj_df.subj_id.unique().tolist(),
                     'params'      : [risk_aversion, loss_aversion, inverse_temp],
                     'tr'          : tr,
                     ...
                     'wSafe'       : w_safe,
                     'negll'       : negll,
                     'bic'         : len(params) * np.log(len(subj_df)) + 2*negll} #K*np.log(len(pred_mood)) + 2*negll}
