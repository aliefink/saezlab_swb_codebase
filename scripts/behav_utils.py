...
def format_all_behav(raw_behav, return_drops=False, drop_bads=False, drop_bads_t1=False,
    ...
    for subj_df in raw_behav:
        ...
        if pt_dir:
            subj_pt_df = pd.read_csv(f'{pt_dir}{subj_id}_pt_df.csv')
            
            pt_vars = list(set(list(subj_pt_df.columns)).difference(list(subj_df.columns)))

            for var in pt_vars:
                # add var data to df 
                subj_df[var] = subj_pt_df[var].values
                # add var_t1 data to df
                var_t1 = '_'.join([var,'t1'])
                subj_df[var_t1] = subj_df[var].tolist()[1:]+[np.nan]
        ...
        if norm:
            ...
            if pt_dir:
                # cont_vars.extend(['P_Gamble','P_Safe','Choice_Prob','risk', 'loss', 'temp'])
                cont_vars.extend(['util_SafeBet','util_HighBet','util_LowBet','util_Gamble',
                                  'Choice_Util','util_SafeBet_t1','util_HighBet_t1','util_LowBet_t1','util_Gamble_t1','Choice_Util_t1'])
